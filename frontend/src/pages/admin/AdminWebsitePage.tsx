...
import { useBarberMutations, useSettingsMutations, useMediaMutations } from '@/features/admin/mutations';
...
import type { Barber, MediaValue, SalonSettings } from '@/types';
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Edit every section shown on the home page. Changes appear immediately. About, Contact, FAQ and Footer
          content are edited in their own tabs.
        </p>
      </div>
      ...
      <div className="flex h-full w-full items-start justify-center sm:items-center sm:p-5">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain px-5 py-5 sm:px-6">
            ...
            {section.key === 'team' && (
              <section className="mb-7">
                <GroupHeader label="Team members" hint="Barbers shown in the team section" />
                <BarbersEditor />
              </section>
            )}
          </div>
...
function BarbersEditor() {
  ...
  const mut = useBarberMutations();
  const [form, setForm] = useState<Partial<Barber> | null>(null);

  const save = () => {
    if (!form?.name) return;
    if (form._id) {
      mut.update.mutate({ id: form._id, data: form }, { onSuccess: () => setForm(null) });
    } else {
      mut.create.mutate(form, { onSuccess: () => setForm(null) });
    }
  };
  ...
  return (
    <div className="col-span-2 rounded-md border border-white/10 p-4">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-semibold text-white">Barbers / team members</p>
        <button
          type="button"
          onClick={() => setForm({ name: '', tamilName: '', specialty: '', isActive: true, sortOrder: 0 })}
          className="inline-flex items-center gap-1.5 rounded-md bg-white/10 px-3 py-1.5 text-xs font-semibold text-white hover:bg-white/20"
        >
          <Plus className="h-3.5 w-3.5" /> Add barber
        </button>
      </div>

      {barbers.length === 0 ? (
        <p className="text-xs text-zinc-500">No barbers yet. Add one to show team members on the home page.</p>
      ) : (
        <div className="grid gap-2 sm:grid-cols-2">
          {barbers.map((b) => (
            <div key={b._id} className="flex items-center justify-between gap-2 rounded-md border border-white/10 p-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-white">{b.name}</p>
                <p className="truncate text-xs text-zinc-500">{b.specialty || b.tamilName || 'General barber'}</p>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <span className={b.isActive ? 'text-xs text-green-400' : 'text-xs text-zinc-600'}>
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (window.confirm(`Remove ${b.name}?`)) mut.remove.mutate(b._id);
                  }}
      ...
      {form && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-md border border-white/10 bg-[#141414] p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-5 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-white">{form._id ? 'Edit barber' : 'New barber'}</h2>
              <button type="button" onClick={() => setForm(null)} className="text-zinc-400 hover:text-white" aria-label="Close">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="grid gap-3">
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium text-zinc-400">Name (required)</span>
                <input className="input-dark" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              </label>
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium text-zinc-400">Tamil name</span>
                <input className="input-dark" value={form.tamilName ?? ''} onChange={(e) => setForm({ ...form, tamilName: e.target.value })} />
              </label>
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium text-zinc-400">Specialty</span>
                <input className="input-dark" value={form.specialty ?? ''} onChange={(e) => setForm({ ...form, specialty: e.target.value })} placeholder="Fades, beards…" />
              </label>
