...
import { ErrorMessage, QuotesSkeleton } from '@/components/ui/Feedback';
...
export function QuotesSection({ source = 'general' }: { source?: 'home' | 'about' | 'contact' | 'general' }) {
  const { data: quotes = [], isLoading, error, refetch } = useQuotes({ source });
  const active = quotes.filter((q) => q.isActive);

  if (!isLoading && !error && active.length === 0) return null;
  ...
  return (
    <section className="container-x py-24 lg:py-32" id="quotes">
      ...
      {isLoading ? (
        <QuotesSkeleton count={2} />
      ) : error ? (
        <div className="mx-auto max-w-md">
          <ErrorMessage message="Could not load quotes." onRetry={() => refetch()} />
        </div>
      ) : (
